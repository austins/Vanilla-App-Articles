...
class ArticlesHooks implements Gdn_IPlugin {
    ...
    /**
     * Handles request methods for the controller that calls it.
     * Based on the Dispatch method from the Gdn_Plugin class of 2.1b2
     * and modified to allow a custom method prefix.
     */
    private function Dispatch($Sender, $RequestArgs, $ControllerName = 'Controller') {
    ...
    public function SettingsController_Articles_Create($Sender) {
        $this->Dispatch($Sender, $Sender->RequestArgs, 'DispatchSettings');
    }

    /**
     * The Index method of the Articles setting page.
     */
    private function DispatchSettings_Index($Sender) {
     ...
     */
    private function DispatchSettings_Categories($Sender) {
